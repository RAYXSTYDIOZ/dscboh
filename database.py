...
class DatabaseManager:
    ...
    def get_user_memory(self, user_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'SELECT profile_summary, vibe, interaction_count FROM user_memory WHERE user_id = ?',
                    (user_id,)
                )
                row = cursor.fetchone()
                if row:
    ...
    def update_user_memory(self, user_id, username, profile_summary=None, vibe=None, notes=None):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT interaction_count FROM user_memory WHERE user_id = ?', (user_id,))
                row = cursor.fetchone()
                
                if row:
                    interaction_count = row[0] + 1
                    updates = ["interaction_count = ?", "last_updated = CURRENT_TIMESTAMP", "username = ?"]
                    params = [interaction_count, username]
                    if profile_summary is not None: updates.append("profile_summary = ?"); params.append(profile_summary)
                    if vibe is not None: updates.append("vibe = ?"); params.append(vibe)
                    if notes is not None: updates.append("notes = ?"); params.append(notes)
                    params.append(user_id)
                    cursor.execute(f"UPDATE user_memory SET {', '.join(updates)} WHERE user_id = ?", params)
                else:
                    cursor.execute(
                        '''INSERT INTO user_memory (user_id, username, profile_summary, vibe, notes, interaction_count) 
                           VALUES (?, ?, ?, ?, ?, 1)''',
    ...
    def delete_guild_inviter(self, guild_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('DELETE FROM guild_inviters WHERE guild_id = ?', (str(guild_id),))
                conn.commit()
        except Exception as e:
    ...
    def delete_captcha(self, user_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('DELETE FROM active_captchas WHERE user_id = ?', (int(user_id),))
                conn.commit()
        except Exception as e:
    ...
    def save_reminder(self, user_id, text, delay):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'INSERT INTO user_reminders (user_id, reminder_text, delay) VALUES (?, ?, ?)',
    ...
    def delete_reminder(self, user_id, text):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('DELETE FROM user_reminders WHERE user_id = ? AND reminder_text = ?', (int(user_id), text))
                conn.commit()
        except Exception as e:
    ...
    def get_notes(self, user_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT note_text FROM user_notes WHERE user_id = ? ORDER BY timestamp DESC', (int(user_id),))
    ...
    def delete_notes(self, user_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('DELETE FROM user_notes WHERE user_id = ?', (int(user_id),))
                conn.commit()
        except Exception as e:
